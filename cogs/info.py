...
class Info(commands.Cog):
    ...
    @commands.slash_command(
        name="info-instagram",
        description="To get details of an Instagram user",
    )
    async def instagram(
        self,
        ctx: disnake.AppCmdInter,
        username=Param(description="Username of the insta user"),
    ):
        await ctx.response.defer()
        req = requests.get(f"https://api.popcat.xyz/instagram?user={username}")
        data = json.loads(req.text)
        try:
            ...
            await ctx.send(
                embed=disnake.Embed(description="No data found"), delete_after=4
            )
