...
class Moderator(commands.Cog):
    ...
    async def kick(
        ...
        embed_dm = disnake.Embed(
            title="Kicked!",
            description=f"You have been kicked from `{ctx.guild.name}` by `{ctx.author.mention}` for **{reason}**",
        )
        embed_dm.set_author(
            name=self.bot.user.display_name, icon_url=f"{self.bot.user.avatar.url}"
        )
    ...
    async def ban(
        ...
        embed_dm = disnake.Embed(
            title="Banned!",
            description=f"You have been banned from `{ctx.guild.name}` by `{ctx.author.mention}` for **{reason}**",
        )
        embed_dm.set_author(
            name=self.bot.user.display_name, icon_url=f"{self.bot.user.avatar.url}"
        )
    ...
    async def unban(
        ...
        embed_dm = disnake.Embed(
            title="Ban Revoked!",
            description=f"You have been unbanned from `{ctx.guild.name}` by `{ctx.author.mention}` for **{reason}**",
        )
        embed_dm.set_author(
            name=self.bot.user.display_name, icon_url=f"{self.bot.user.avatar.url}"
        )
    ...
    async def timeout(
        ...
        embed_dm = disnake.Embed(
            title="Timed-out!",
            description=f"You have been timed-out in `{ctx.guild.name}` by `{ctx.author.mention}` for **{reason}**",
        )
        embed_dm.set_author(
            name=self.bot.user.display_name, icon_url=f"{self.bot.user.avatar.url}"
        )
        ...
        if member == ctx.author:
            await ctx.send(
                embed=disnake.Embed(description="Mad? timing-out yourselves?"),
                delete_after=3,
            )
        else:
            try:
    ...
    async def poll(
        ...
        embed.set_footer(text="Poll ID: {}".format(msg.id))
