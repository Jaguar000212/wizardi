...
class Anime(commands.Cog):
    ...
    async def info(
        ...
        try:
            ...
            await ctx.send(
                embed=disnake.Embed(
                    description=f"{self.bot.icons['redtick']} No corresponding Anime is found for the search query",
    ...
    async def character(
        ...
        try:
            ...
            await ctx.send(
                embed=disnake.Embed(
                    description=f"{self.bot.icons['redtick']} No corresponding Anime Character is found for the search query",
                    color=disnake.Colour.red(),
                ),
                delete_after=5,
            )
    ...
    @commands.guild_only()
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.slash_command(name="emote-fox-girl", description="Post a fox-girl gif")
    async def fox_girl(self, ctx: disnake.AppCmdInter):
        await ctx.response.defer()
