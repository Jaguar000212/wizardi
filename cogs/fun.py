...
class Fun(commands.Cog):
    ...
    async def akinator(self, ctx: disnake.AppCmdInter):
        ...
        while aki.progression <= 80:
            ...
            def check(react:disnake.Reaction, user:disnake.User):
                if user == ctx.author and react.message.id == ques.id and (react.emoji in ["👍🏻", "👎🏻", "🤔", "🤷🏻", "🔙"]):
                    return True
                return False
            try:
        ...
        def check(react:disnake.Reaction, user:disnake.User):
            if user == ctx.author and react.message.id == ans_msg.id and (react.emoji in ["👍🏻", "👎🏻"]):
                return True
            return False
        try:
            reaction, user = await self.bot.wait_for("reaction_add", check=check, timeout=30)
    ...
    async def tic(
        ...
        embed.description = f"`Challenger` - {ctx.author.mention} (<:z_BOT_online:945976489420128256>)\n`Opponent` - {player.mention} (<:z_BOT_error:946056945935192114>)\n\n```diff\n- Current Turn - {player}```"
